...
function App() {
  ...
  async function checkInByQr(qrCode) {
    ...
    if (error || !data) return setNotice({ type: 'error', title: 'QR Code Not Found', body: qrCode })
    if (data.status === 'checked_in') return setNotice({ type: 'warning', title: 'Already Checked In', body: `${data.name} • ${new Date(data.check_in_time).toLocaleTimeString()}` })

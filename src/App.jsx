...
import { Camera, Download, FileUp, Pencil, Plus, Search, Trash2, Users } from 'lucide-react'
import './style.css'
...
function App() {
  ...
  function showScanFeedback(feedback) {
  ...
  const groups = useMemo(() => {
    const set = new Set(attendees.map(a => a.team).filter(t => t && t.trim()))
    return Array.from(set).sort()
  }, [attendees])

  async function updateGroup(attendee, newGroup) {
    if (newGroup === '__new__') {
      const typed = window.prompt('New group name:')
      if (!typed || !typed.trim()) return
      newGroup = typed.trim()
    }
    const { data, error } = await supabase.from('attendees').update({ team: newGroup }).eq('id', attendee.id).select().single()
    if (error) return alert(error.message)
    setAttendees(prev => prev.map(a => a.id === data.id ? data : a))
  }

  async function renameGroup(oldName) {
    const typed = window.prompt(`Rename group "${oldName}" to:`, oldName)
    if (!typed || !typed.trim() || typed.trim() === oldName) return
    const newName = typed.trim()
    const { data, error } = await supabase.from('attendees').update({ team: newName }).eq('event_id', eventId).eq('team', oldName).select()
    if (error) return alert(error.message)
    setAttendees(prev => prev.map(a => a.team === oldName ? { ...a, team: newName } : a))
    setNotice({ type: 'success', title: 'Group Renamed', body: `${oldName} → ${newName} (${data.length} attendees)` })
  }

  async function deleteGroup(name) {
    const sure = window.confirm(`Remove the group "${name}"?\n\nAttendees in this group will become "No Group" — they will NOT be deleted.`)
    if (!sure) return
    const { data, error } = await supabase.from('attendees').update({ team: null }).eq('event_id', eventId).eq('team', name).select()
    if (error) return alert(error.message)
    setAttendees(prev => prev.map(a => a.team === name ? { ...a, team: null } : a))
    setNotice({ type: 'warning', title: 'Group Removed', body: `${name} (${data.length} attendees now ungrouped)` })
  }

  async function checkInByQr(qrCode) {
    const { data, error } = await supabase.from('attendees').select('*').eq('qr_code', qrCode).maybeSingle()
    if (error || !data) { playBeep('error'); showScanFeedback({ type: 'error', name: 'Not Found', message: qrCode }); return setNotice({ type: 'error', title: 'QR Code Not Found', body: qrCode }) }
    if (data.status === 'checked_in') { playBeep('error'); showScanFeedback({ type: 'error', name: data.name, team: data.team, message: 'Already Checked In' }); return setNotice({ type: 'warning', title: 'Already Checked In', body: `${data.name} • ${new Date(data.check_in_time).toLocaleTimeString()}` }) }
    await manualCheckIn(data)
    playBeep('success')
    showScanFeedback({ type: 'success', name: data.name, team: data.team, message: 'Checked In' })
  }
  ...
  async function undoCheckIn(attendee) {
    const sure = window.confirm(`Undo check-in for ${attendee.name}?`)
    if (!sure) return
    const { data, error } = await supabase.from('attendees').update({ status: 'pending', check_in_time: null, checked_by: null }).eq('id', attendee.id).select().single()
    if (error) return alert(error.message)
    setAttendees(prev => prev.map(a => a.id === data.id ? data : a))
    setNotice({ type: 'warning', title: 'Check-in Undone', body: data.name })
  }
  ...
  return <div className="app">
    ...
    {activeTab === 'scan' && <div className="scan-wrap">
      <Scanner onScan={checkInByQr} />
      {scanFeedback && <div className={`scan-feedback ${scanFeedback.type}`}>
        <div className="sf-name">{scanFeedback.name}</div>
        {scanFeedback.type === 'success' && <div className="sf-team">{scanFeedback.team || 'No Group'}</div>}
        {scanFeedback.type !== 'success' && scanFeedback.team && <div className="sf-team-small">{scanFeedback.team}</div>}
        {scanFeedback.message && <div className="sf-message">{scanFeedback.message}</div>}
      </div>}
    </div>}

    {activeTab === 'attendees' && <section>
      <div className="search"><Search size={18}/><input placeholder="Search name / phone / team" value={search} onChange={e=>setSearch(e.target.value)}/></div>

      {groups.length > 0 && <div className="manage-groups">
        <h3>Manage Groups</h3>
        <div className="group-chips">
          {groups.map(g => <div className="group-chip" key={g}>
            <span>{g}</span>
            <button className="chip-btn" onClick={()=>renameGroup(g)} title="Rename group"><Pencil size={13}/></button>
            <button className="chip-btn danger" onClick={()=>deleteGroup(g)} title="Delete group"><Trash2 size={13}/></button>
          </div>)}
        </div>
      </div>}

      <div className="walkin"><h3>Add Walk-in</h3><input placeholder="Full Name" value={walkIn.name} onChange={e=>setWalkIn({...walkIn,name:e.target.value})}/><input placeholder="Phone Number" value={walkIn.phone} onChange={e=>setWalkIn({...walkIn,phone:e.target.value})}/><input placeholder="Email Address" value={walkIn.email} onChange={e=>setWalkIn({...walkIn,email:e.target.value})}/><button onClick={addWalkIn}>Add + Check-in</button></div>
      <div className="list">{filtered.map(a => <div className="row" key={a.id}><div><b>{a.name}</b><small>{a.phone}</small></div><select className="group-select" value={a.team || ''} onChange={e=>updateGroup(a, e.target.value)}><option value="">No Group</option>{groups.map(g => <option key={g} value={g}>{g}</option>)}<option value="__new__">+ Add New Group</option></select><span className={a.status}>{a.status === 'checked_in' ? 'Checked In' : 'Pending'}</span>{a.status !== 'checked_in' && <button onClick={()=>manualCheckIn(a)}>Check In</button>}{a.status === 'checked_in' && <button className="undo" onClick={()=>undoCheckIn(a)}>Undo</button>}</div>)}</div>
    </section>}
